(() => {
  ...
  // Cache-buster version; bumped only when the admin uploads a new video
  let _videoCbVersion = null;

  const resolveHeroVideoUrl = (cfgUrl) => {
    const base = cfgUrl || '/moveathens/videos/hero.mp4';
    // Use a stable cache-buster per session so the browser can cache the 2.5 MB file
    // across navigations, but still picks up a new upload after a page reload.
    if (!_videoCbVersion) _videoCbVersion = Date.now();
    return `${base}?v=${_videoCbVersion}`;
  };
  ...
  const applyHero = async (root, cfg) => {
    ...
    if (logo) {
      if (cfg.heroLogoUrl) {
        // Prefer WebP if available (auto-generated by server), fall back to original
        const webpUrl = cfg.heroLogoUrl.replace(/\.(png|jpe?g)$/i, '.webp');
        if (webpUrl !== cfg.heroLogoUrl) {
          logo.src = webpUrl;
          logo.onerror = () => { logo.onerror = null; logo.src = cfg.heroLogoUrl; };
        } else {
