...
(() => {
  ...
  // ========================================
  // CATEGORY STYLE — apply CSS variables from config
  // ========================================
  const applyCategoryStyle = () => {
    const s = CONFIG?.categoryStyle;
    if (!s) return;
    const root = document.documentElement;
    const scale = parseFloat(s.tileScale) || 1;
    if (scale !== 1) {
      root.style.setProperty('--ma-tile-scale', scale);
    }
  ...
  const renderCategories = async () => {
    ...
    // Apply category style from config
    applyCategoryStyle();
