...
const MODE_DEFAULT_META = {
  ...
  mercedes: { capacity: 1, charge_type: 'per_vehicle', label: 'Private Mercedes' },
...
function baseModeTemplate(modeKey) {
  const meta = MODE_DEFAULT_META[modeKey] || { capacity: 0, charge_type: 'per_person', label: modeKey };
  return {
    active: false,
    title: '',
    subtitle: '',
...
function loadTripTemplate() {
  ...
  try {
    ...
    const fallbackModes = {};
    MODE_KEYS.forEach((key) => { fallbackModes[key] = baseModeTemplate(key); });
    tripTemplateCache = {
      id: '',
      slug: '',
      title: '',
...
function normalizeStopsList(value) {
  if (typeof value === 'string') {
    return value.split(/\r?\n|,/g).map((line) => ({ title: line.trim() })).filter((entry) => entry.title);
  }
  if (!Array.isArray(value)) return [];
  return value.map((entry) => {
    ...
    if (!title && !description && !images.length && !videos.length) return null;
    return { title, description, images, videos };
  }).filter(Boolean);
}
...
function normalizeMapPoint(value) {
  ...
  if (typeof value === 'string') return { label: value.trim(), lat: null, lng: null };
...
function normalizeRouteList(value) {
  if (Array.isArray(value)) return value.map(normalizeMapPoint).filter((pt) => pt.label || (pt.lat !== null && pt.lng !== null));
  if (typeof value === 'string') {
    return value.split(/\r?\n/g).map((line) => {
...
function normalizeMapBlock(value) {
  ...
  if (!route.length && markers.length) {
    route.push(...markers.map(normalizeMapPoint).filter((pt) => pt.label || (pt.lat !== null && pt.lng !== null)));
  }
  return {
...
function normalizeChargeType(value, fallback) {
  const val = String(value || fallback || 'per_person').trim().toLowerCase();
...
function normalizeModeBlock(modeKey, input, fallbackTitle) {
  ...
  const defaultTitle = fallbackTitle ? `${fallbackTitle} – ${meta.label}` : meta.label;
  const block = {
  ...
  if (!block.video.url && raw.videoUrl) block.video.url = String(raw.videoUrl).trim();
  if (!block.video.thumbnail && raw.videoThumbnail) block.video.thumbnail = String(raw.videoThumbnail).trim();
  return block;
}
...
function deriveModeSetFromModes(modes) {
  ...
  MODE_KEYS.forEach((key) => {
    ...
    const charge = block.charge_type === 'per_vehicle' ? 'per_vehicle' : 'per_person';
    let priceSource = (charge === 'per_vehicle') ? block.price_total : block.price_per_person;
    if (priceSource === null || typeof priceSource === 'undefined') {
      priceSource = (charge === 'per_vehicle') ? block.price_per_person : block.price_total;
    }
    const priceCents = Number.isFinite(priceSource) ? Math.max(0, Math.round(priceSource * 100)) : 0;
    out[key] = {
      active: !!block.active,
      price_cents: priceCents,
      charge_type: charge,
      default_capacity: toInt(block.capacity, MODE_DEFAULT_META[key] ? MODE_DEFAULT_META[key].capacity : 0)
    };
  });
  return out;
}
...
function resolveDefaultMode(trip) {
  ...
  if (trip && trip.modes && trip.modes[preferred] && trip.modes[preferred].active) return preferred;
  const active = MODE_KEYS.find((key) => trip && trip.modes && trip.modes[key] && trip.modes[key].active);
  return active || 'van';
}
...
function projectBaseFieldsFromDefaultMode(trip) {
  ...
  if ((!trip.tags || !trip.tags.length) && Array.isArray(mode.tags)) trip.tags = mode.tags.slice();
  return trip;
}
...
function migrateLegacyTrip(trip) {
  ...
  MODE_KEYS.forEach((key) => {
    trip.modes[key] = normalizeModeBlock(key, trip.modes[key], trip.title || trip.slug || 'Trip');
  });
  const copyArrayField = (field, transform) => {
    const source = transform ? transform(trip[field]) : trip[field];
    const hasSource = Array.isArray(source) ? source.length > 0 : !!(source && Object.keys(source).length);
    if (!hasSource) return;
    MODE_KEYS.forEach((key) => {
      const block = trip.modes[key];
      const target = block[field];
      const hasTarget = Array.isArray(target) ? listHasMeaningfulEntries(target) : !!(target && Object.keys(target).length);
  ...
  if (trip.video) {
    ...
    MODE_KEYS.forEach((key) => {
      const block = trip.modes[key];
      if (!block.video || (!block.video.url && !block.video.thumbnail)) block.video = cloneJson(normalized);
    });
    delete trip.video;
  }
  if (trip.map) {
    ...
    MODE_KEYS.forEach((key) => {
      const block = trip.modes[key];
      if (!block.map || !listHasMeaningfulEntries(block.map.route)) block.map = cloneJson(normalizedMap);
    });
    delete trip.map;
  }
  ...
  if (modeSet && typeof modeSet === 'object') {
    MODE_KEYS.forEach((key) => {
      ...
      if (!block.capacity && meta.default_capacity) block.capacity = meta.default_capacity;
...
function cleanupLegacyFields(trip) {
  ['stops', 'sections', 'includes', 'excludes', 'gallery', 'videos', 'faq', 'video', 'map'].forEach((field) => {
...
function ensureTripsDir() {
  try { fs.mkdirSync(TRIPS_DIR, { recursive: true }); } catch (_) { /* ignore */ }
  try { fs.mkdirSync(UPLOAD_TRIPS_DIR, { recursive: true }); } catch (_) { /* ignore */ }
}
...
function validateTrip(input) {
  ...
  activeModes.forEach((key) => {
    ...
    if (!block.duration && !block.duration_hours && !block.duration_days) errors.push(`mode_${key}_missing_duration`);
    const charge = block.charge_type === 'per_vehicle' ? 'per_vehicle' : 'per_person';
    if (charge === 'per_person' && block.price_per_person == null) errors.push(`mode_${key}_missing_price`);
    if (charge === 'per_vehicle' && block.price_total == null) errors.push(`mode_${key}_missing_price`);
    if (!listHasMeaningfulEntries(block.stops)) errors.push(`mode_${key}_missing_stops`);
...
function registerTripsRoutes(app, { checkAdminAuth }) {
  ...
  adminRouter.post('/', express.json({ limit: '400kb' }), (req, res) => {
    if (!checkAdminAuth || !checkAdminAuth(req)) return res.status(403).json({ error: 'Forbidden' });
    const input = (req.body && typeof req.body === 'object') ? req.body : {};
    const validation = validateTrip(input);
    if (!validation.ok) return res.status(400).json({ error: 'validation_failed', errors: validation.errors });
...
function baseModeTemplate(modeKey){
  const meta = MODE_DEFAULT_META[modeKey] || { capacity: 0, charge_type: 'per_person' };
  return {
    active: false,
    title: '',
    subtitle: '',
