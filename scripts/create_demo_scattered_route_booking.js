#!/usr/bin/env node
/**
 * Create a DEMO booking with scattered stops to test Google optimization & ETAs.
 * Default driver: testdriver@greekaway.com
 *
 * Stops (user request, intentionally scattered):
 * 1) Πλατεία Νυμφών, Γλυφάδα
 * 2) Πλατεία Νέας Σμύρνης, Νέα Σμύρνη
 * 3) Ιπποκράτους 43, Αθήνα
 * 4) Μουσείο Ακρόπολης (Acropolis Museum, Αθήνα)
 * 5) Εστιατόριο Βόσπορος, Μικρολίμανο, Πειραιάς
 *
 * Usage:
 *   node scripts/create_demo_scattered_route_booking.js [--driver-email <email>] [--date YYYY-MM-DD]
 */
...
function buildStops(){
  return [
    { name: 'Γλυφάδα', address: 'Πλατεία Νυμφών, Γλυφάδα', time: '08:00' },
    { name: 'Νέα Σμύρνη', address: 'Πλατεία Νέας Σμύρνης, Νέα Σμύρνη', time: null },
    { name: 'Αθήνα (Κέντρο)', address: 'Ιπποκράτους 43, Αθήνα', time: null },
    { name: 'Μουσείο Ακρόπολης', address: 'Acropolis Museum, Αθήνα', time: null },
    { name: 'Βόσπορος (Μικρολίμανο)', address: 'Εστιατόριο Βόσπορος, Μικρολίμανο, Πειραιάς', time: null }
  ];
}
...
function buildBooking(driverId, date){
  ...
  return {
    ...
    event_id: `DEMO_SCAT_${id.slice(0,8)}`,
    user_name: 'Demo Scattered Route',
    user_email: 'demo-scattered@example.com',
    trip_id: 'DEMO_SCATTERED_GOOGLE_TEST',
    seats: 3,
    price_cents: 0,
    currency: 'EUR',
    ...
    pickup_lng: null,
    suitcases_json: '[]',
    special_requests: 'Google optimization & ETAs demo',
    created_at,
