/**
 * DriversSystem — Tax / Insurance Expenses Page
 * Two-mode page based on URL:
 *   /tax-expenses          → Level 1: list of groups (navigate via links)
 *   /tax-expenses/:groupId → Level 2: items (toggle-select) + ATM amount + notes
 */
(async () => {
  'use strict';
  ...
  const groupIdFromUrl = (lastPart !== 'tax-expenses') ? lastPart : null;

  // ── DOM refs ──
  const groupsSection = $('[data-ds-tax-exp-groups]');
  const itemsSection  = $('[data-ds-tax-exp-items]');
  const itemsGrid     = $('[data-ds-tax-exp-items-grid]');
  const amountInput   = $('[data-ds-tax-exp-amount]');
  const noteInput     = $('[data-ds-tax-exp-note]');
  const saveBtn       = $('[data-ds-tax-exp-save]');
  const msgEl         = $('[data-ds-tax-exp-msg]');
  const backBar       = $('[data-ds-tax-exp-back-bar]');
  const titleEl       = $('[data-ds-tax-exp-title]');
  ...
  const loadCategories = async () => {
    try {
      const res = await fetch('/api/driverssystem/tax-expense-categories');
  ...
  const renderGroupsList = () => {
    ...
    if (categories.length === 0) {
      groupsSection.innerHTML = '<div class="ds-tax-exp-empty">Δεν υπάρχουν κατηγορίες</div>';
      return;
    }
    const base = window.DriversSystemConfig.buildRoute('/tax-expenses');
    groupsSection.innerHTML = categories.map((group) => `
      <a class="ds-tax-exp-group-btn" href="${base}/${group.id}">
        <span class="ds-tax-exp-group-btn__name">${group.name}</span>
        <span class="ds-tax-exp-group-btn__count">${(group.items || []).length} είδη</span>
      </a>
    `).join('');
  };
  ...
  const renderItems = (group) => {
    ...
    if (items.length === 0) {
      itemsGrid.innerHTML = '<div class="ds-tax-exp-empty">Κανένα είδος σε αυτή την ομάδα</div>';
      return;
    }
    itemsGrid.innerHTML = items.map((item, i) => `
      <button class="ds-tax-exp-item-btn" data-item-idx="${i}">
        ${item.name}
      </button>
    `).join('');
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const res = await fetch('/api/driverssystem/tax-expenses', {
          method: 'POST',
  ...
  const backBtn = $('[data-ds-tax-exp-back]');
  if (backBtn) {
